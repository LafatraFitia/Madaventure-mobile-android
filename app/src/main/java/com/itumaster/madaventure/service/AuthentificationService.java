...
public class AuthentificationService {
    ...
    public static void login(Utilisateur utilisateur, final ApiResponseListener<Utilisateur> listener) {
        String url = "login";
        String jsonBody = "{\"email\":\"" + utilisateur.getEmail() + "\",\"password\":\"" + utilisateur.getPassword() + "\"}";

        NetworkUtils.performPostRequestAsync(BASE_URL + url, jsonBody, new NetworkUtils.NetworkCallback() {
            @Override
            public void onSuccess(String response) {
                Log.d("Authentification", "Response: " + response);
                ApiResponse<Utilisateur> apiResponse = ApiResponse.fromJson(response, Utilisateur.class);
